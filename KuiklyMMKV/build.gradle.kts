...
kotlin {
    ...
    // Kuikly Core Group ID（支持外网 kuikly-open / 内网 kuikly 双版本）
    val coreGroup = project.kuiklyCoreGroup()

    sourceSets {
        val commonMain by getting {
            dependencies {
                implementation("${coreGroup}:core:${Version.getKuiklyVersion()}")
                implementation("${coreGroup}:core-annotations:${Version.getKuiklyVersion()}")
            }
        }
